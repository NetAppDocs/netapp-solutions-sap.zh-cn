...
有关使用 SnapCenter 的 SAP 系统刷新操作的详细信息，请参见 https://docs.netapp.com/us-en/netapp-solutions-sap/lifecycle/sc-copy-clone-introduction.html["利用 SnapCenter 自动执行 SAP HANA 系统复制和克隆操作"]。
...
如"先决条件"部分所述，对于 SnapCenter 版本 < 6.2，必须使用命令"systemctl restart spl"重新启动 HANA 主机上的 SnapCenter spl 服务，以启动正确的清理。
