...
已为 HANA 系统数据库创建了一个名为“SnapCenter”的数据库用户，并赋予其执行备份和恢复操作所需的权限（参见）。 link:hana-sc-generic-scope.html["使用 SnapCenter 实现 SAP HANA 备份和恢复"^]）。必须在两台主机上使用上述数据库用户配置 HANA 用户存储密钥。
...
SnapCenter 4.6 资源组配置的恢复操作与自动发现的非系统复制设置相同。所有恢复和自动恢复选项均可用。更多详情请参阅技术报告。 link:hana-sc-generic-scope.html["使用 SnapCenter 实现 SAP HANA 备份和恢复"^]。
