...
NOTE: 使用 Azure NetApp Files，主 ANF 卷或 ANF 备份支持还原操作。对于主 ANF 卷，将执行卷还原；对于 ANF 备份，将执行具有单个文件还原的就地还原。在这两种情况下，都将保持应用程序卷组配置。


NOTE: 如果启用了卷加密并使用了 SAP 本地安全存储 (LSS)，则只要自备份以来 LSS 中的根密钥备份密码未更改，即支持使用 SnapCenter 进行恢复。如果更改了密码，并且使用具有不同密码的旧 Snapshot 完成了还原和恢复，则需要手动完成恢复，并且需要在恢复语句中提供旧密码："RECOVER DATA USING SNAPSHOT CLEAR LOG ENCRYPTION ROOT KEYS BACKUP PASSWORD 'old-password'"
