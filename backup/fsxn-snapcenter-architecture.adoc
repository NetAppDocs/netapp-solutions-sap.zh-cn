...
本文档描述了在 FSx for ONTAP上具有单个租户的 SAP HANA MDC 单主机系统最常见的SnapCenter配置选项。对于特定的 SAP HANA 系统，例如多主机系统，还可能存在其他配置选项，并且在某些情况下，这些选项是必需的。有关其他配置选项的详细说明，请参阅 link:hana-sc-generic-concepts.html["SnapCenter 概念和最佳实践(netapp.com)"^]。
