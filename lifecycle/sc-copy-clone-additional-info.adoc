...
* link:../bp/hana-aff-fc-introduction.html["采用光纤通道协议的 NetApp 全闪存 FAS 系统上的 SAP HANA"]
* link:../bp/hana-aff-nfs-introduction.html["基于 NFS 的 NetApp 全闪存 FAS 系统上的 SAP HANA"]
* link:../backup/fsxn-overview.html["基于Amazon FSX的SAP HANA for NetApp ONTAP —使用SnapCenter 进行备份和恢复"]
* link:lama-ansible-introduction.html["使用Ansible实现NetApp SAP Landscape Management集成"]
* link:libelle-sc-overview.html["通过 Libelle SystemCopy (netapp.com/cn) 自动化 SAP 系统复制操作"]
...
| 4.1 版 | 2026 年 2 月 | 添加了 HANA 加密和 SAP 本地安全存储 LSS 的必要步骤 
|===