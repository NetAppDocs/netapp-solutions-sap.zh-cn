...
---
= 利用Libelle SystemCopy自动执行SAP系统副本操作
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
虽然维修系统和灾难恢复测试系统通常使用 SAP 系统克隆（不需要大量的后期处理操作）来更新测试和培训系统，但必须应用这些后期处理步骤来实现与源系统的共存。因此，本文档侧重于 SAP 系统更新场景。有关不同用例的更多详细信息，请参见技术报告 link:sc-copy-clone-introduction.html["利用 SnapCenter 自动执行 SAP HANA 系统复制和克隆操作"^]。
