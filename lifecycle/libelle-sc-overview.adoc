...
虽然修复系统和灾难恢复测试系统通常使用SAP系统克隆(不需要执行大量后处理操作)来更新测试和培训系统、但要与源系统共存、必须应用这些后处理步骤。因此、本文档重点介绍SAP系统更新场景。有关不同使用情形的更多详细信息、请参见技术报告 https://www.netapp.com/pdf.html?item=/media/17111-tr4667pdf.pdf["TR-4667：使用SnapCenter 自动执行SAP HANA系统复制和克隆操作"^]。
