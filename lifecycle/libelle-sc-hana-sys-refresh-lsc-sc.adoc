...
有关这些不同的SnapCenter代理和 SAP HANA 数据库插件安装选项的更多详细信息，请参阅技术报告。 link:../backup/hana-sc-generic-scope.html["使用 SnapCenter 进行 SAP HANA 备份和恢复"^]。

以下各节重点介绍了使用标准安装或中央通信主机将LSC与SnapCenter 集成的区别。值得注意的是、无论使用何种安装选项和数据库、未突出显示的所有配置步骤都是相同的。

要从源数据库执行基于快照副本的自动备份并为新目标数据库创建克隆，所描述的 LSC 与 SnapCenter 之间的集成使用 link:sc-copy-clone-introduction.html["利用 SnapCenter 自动执行 SAP HANA 系统复制和克隆操作"^] 中所述的配置选项和脚本。
...
* https://www.netapp.com/pdf.html?item=/media/12403-tr4700.pdf["适用于 Oracle 数据库的 SnapCenter 插件"^]
* link:../backup/hana-sc-generic-scope.html["使用 SnapCenter 实现 SAP HANA 备份和恢复"^]
* link:sc-copy-clone-introduction.html["利用 SnapCenter 自动执行 SAP HANA 系统复制和克隆操作"^]
