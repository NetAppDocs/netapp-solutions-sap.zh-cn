...
所有后续步骤均与""一节所述的从主存储克隆的工作流相同<<Cloning from primary storage with tenant name equal to SID>>。 "
...
SnapCenter 中所需的步骤与""一节中所述的步骤相同<<Cloning from primary storage with tenant name equal to SID>>。"] 不同之处在于脚本`sc-system-refresh.sh`中的租户恢复操作。
