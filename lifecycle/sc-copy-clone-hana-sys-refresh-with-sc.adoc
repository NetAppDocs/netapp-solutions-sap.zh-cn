...
|  | SAP HANA持久性加密 | SFSS：最初，或者如果在源系统上更改了根密钥，则必须在目标系统上导入根密钥备份，然后才能执行恢复。LSS：最初，或者如果根密钥备份密码已更改，则必须在 LSS 配置中设置密码。 
...
|  | SAP HANA持久性加密 | SFSS：如果目标系统之前不存在源系统的 DBID，则必须先恢复系统数据库，然后才能导入此租户的根密钥备份。LSS：在租户数据库恢复期间，LSS 拓扑将自动与 HANA 拓扑同步。 
...
所有 SAP HANA 系统都必须按照配置指南进行配置。 link:../bp/hana-aff-nfs-introduction.html["采用NFS的NetApp AFF 系统上的SAP HANA"]。SnapCenter和 SAP HANA 资源均按照最佳实践指南进行配置。 link:../backup/hana-sc-generic-scope.html["使用 SnapCenter 实现 SAP HANA 备份和恢复"]。
...
. 安装SAP HANA目标系统
. 按照SnapCenter中的描述配置 SAP HANA 系统 link:../backup/hana-sc-generic-scope.html["使用 SnapCenter 实现 SAP HANA 备份和恢复"]
+
...
在光纤通道 SAN 设置中，需要在下一个屏幕中提供使用的协议。
