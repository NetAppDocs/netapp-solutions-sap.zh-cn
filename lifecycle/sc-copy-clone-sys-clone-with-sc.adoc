...
+
如果不再需要系统，则通过以下步骤执行清理过程：

+
.. 对 SAP HANA 数据卷执行 SnapCenter 克隆删除工作流，包括数据库关闭。
+
[NOTE]
====
默认情况下，SnapCenter 克隆删除工作流不会删除 `/etc/fstab`条目。

为了在克隆删除过程中自动删除  `/etc/fstab` 条目，必须首先恢复克隆并使其成为自动发现的资源。如果未执行此步骤， `/etc/fstab` 条目不会自动删除，需要手动删除。

====
.. 停止SAP HANA服务。
.. 对 SAP HANA 共享卷执行 SnapCenter 克隆删除工作流。
