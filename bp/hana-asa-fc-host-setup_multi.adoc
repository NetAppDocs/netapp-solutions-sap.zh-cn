...
NOTE: SAP HANA 多主机系统需要 `/hana/shared`文件系统和 `/lss/shared`文件系统，如果使用 SAP Local Secure Store service(LSS)，则需要连接到系统的所有主机。通常，这是由 NFS 服务器提供的 NFS 共享。建议使用高可用性的 NFS 服务器，例如 NetApp FAS 或 AFF 系统。另一种选择是为此使用 LINUX 主机的内置 NFS 服务器。
...
要挂载文件系统，请运行以下命令： `mount -a` 在每台主机上执行命令。
