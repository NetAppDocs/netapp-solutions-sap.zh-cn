...
. 检查磁盘架配置是否正确，如 " shelf connection。 "
. 创建并配置所需的聚合，如 " configuration。 "
. 创建 Storage Virtual Machine （ SVM ），如 " virtual machine configuration。 "
. 创建逻辑接口（ LIF ），如 " interface configuration。 "
. 创建端口集，如 " port sets。 "
. 在聚合中创建启动程序组，卷和 LUN ，如创建 " and volumes and mapping LUNs to initiator groups。 "
