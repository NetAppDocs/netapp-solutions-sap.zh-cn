...
下图显示了 4+1 多主机 SAP HANA 系统的卷配置。每个 SAP HANA 主机的数据卷和日志卷会分布到不同的存储控制器。例如，在控制器 A 上配置了卷 `SID_data_mnt00001` ，在控制器 B 上配置了卷 `SID_log_mnt00001`每个卷都配置了一个 LUN 。
 ...
 `/hana/shared` 卷必须可由所有 HANA 主机访问，因此使用 NFS 导出。即使 `/hana/shared`文件系统没有特定的性能 KPI，NetApp 也建议使用 10Gb 以太网连接。
...
NOTE: 如果使用 SAP 本地安全存储服务 (LSS)，则所有主机都必须可以访问该  `/lss/shared/SID` 目录。NetApp 建议为该  `/lss/shared/SID` 目录创建一个卷，并使用 NFS 在所有主机上挂载该卷。
...
LVM 卷组的 LUN 应存储在不同的聚合和不同的控制器中。下表显示了 2+1 SAP HANA 多主机系统的每个卷组的两个 LUN 的示例。
