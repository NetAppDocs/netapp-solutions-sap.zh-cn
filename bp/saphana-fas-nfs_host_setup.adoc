...
。 `sunrpc.tcp_max_slot_table_entries` 参数必须在中设置 `/etc/modprobe.d/sunrpc.conf`。如果此文件不存在，则必须先通过添加以下行来创建它：
...
然后重新启动。此外、还必须调整服务器端值、并按照中所述设置会话插槽的数量 link:saphana-fas-nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["NFSv4 的 NFS 配置。"]
