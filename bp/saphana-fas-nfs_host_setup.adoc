...
NOTE: 性能测试显示、`nconnect=8`可提供良好的读取结果、尤其是对数据卷。日志写入可能会因会话数较少而受益，例如 `nconnect=2` 。使用"nconnect"选项也可能有利于共享卷。请注意，从 NFS 服务器进行的首次挂载（ IP 地址）会定义所使用的会话量。即使nconnect使用不同的值、进一步挂载到同一IP地址也不会更改此设置。
