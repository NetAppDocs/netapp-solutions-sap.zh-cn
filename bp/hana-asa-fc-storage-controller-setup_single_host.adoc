...
下表显示了单主机 SAP HANA 系统的 LUN 配置。

|===
| 目的 | LUN | LVM 逻辑卷 


| 数据 | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| 日志 | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| 已共享 | FC5_shared | 不适用 
|===

NOTE: 使用所述配置，存储用户 SID1adm 的默认主目录的 `/usr/sap/SID1`目录位于本地磁盘上。在使用基于磁盘的复制的灾难恢复设置中，NetApp 建议为 `/usr/sap/SID1`目录创建一个额外的 LUN，以便所有文件系统都在中央存储上。


NOTE: 如果使用 SAP 本地安全存储服务 (LSS)，则 `/lss/shared/SID`目录（LSS 的默认安装目录）位于本地磁盘上。在使用基于磁盘的复制的灾难恢复设置中，NetApp 建议为 `/lss/shared/SID`目录创建一个额外的 LUN，以便所有文件系统都在中央存储上。
