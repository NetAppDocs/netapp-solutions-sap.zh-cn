...
由 ONTAP Autonomous Ransomware Protection 创建的快照不能用于 SAP HANA 数据库的前向恢复。因此，不建议将其用于 SAP HANA 数据和日志卷，应为这些卷禁用此功能。使用基于 SnapCenter 的数据保护和防篡改快照来进行勒索软件防护：link:../backup/hana-sc-generic-scope.html["使用 SnapCenter 进行 SAP HANA 备份和恢复"]。ONTAP Autonomous Ransomware Protection 可能对其他卷（如 /hana/shared）有益。

可以在 ONTAP 集群的卷创建期间禁用此功能的自动激活，但建议保留默认设置，并在不需要此功能的卷上禁用它。
...
NOTE: 如果使用 SAP Local Secure Store 服务 (LSS)，则  `/lss/shared/SID` 目录（LSS 的默认安装目录）位于本地磁盘上。在使用基于磁盘的复制的灾难恢复设置中，NetApp 建议在  `SID_shared` 卷内为  `/lss/shared/SID` 目录创建一个目录，以便所有文件系统都在中央存储上。
...
| SID 共享 | usr-sap + shared + lss-shared | /usr/sap/SID + /hana/shared + /lss/shared/SID 
|===
...
NOTE: 如果使用 SAP 本地安全存储服务 (LSS)，则所有主机都必须可以访问该  `/lss/shared/SID` 目录。NetApp 建议在  `SID_shared` 卷中为该  `/lss/shared/SID` 目录创建一个目录。
...
| sid_shared | lss-shared | /lss/shared/SID | 已挂载到所有主机上 
...
| 禁用勒索软件保护，SID_shared 卷除外 | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled 
