...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf`。此外、还必须调整服务器端值、并按照中所述设置会话插槽的数量 link:saphana_aff_nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["NFSv4 的 NFS 配置。"]
