...
image:saphana_aff_fc_image2a.png["八个连接到一个存储HA对的SAP HANA主机"]
...
image:saphana_aff_fc_image3a.png["连接到存储HA对的其他SAP HANA主机"]

与部署的 AFF 系统无关， SAP HANA 环境也可以通过添加任何经过认证的存储控制器来进行扩展，以满足所需的节点密度，如下图所示。

image:saphana_aff_fc_image4a.png["正在添加其他存储HA对"]
...
image:saphana_aff_fc_image5.jpg["图中显示了输入/输出对话框或表示已写入内容"]
...
image:saphana_aff_fc_image6.png["图中显示了输入/输出对话框或表示已写入内容"]
...
image:saphana_aff_fc_image7.png["图中显示了输入/输出对话框或表示已写入内容"]
