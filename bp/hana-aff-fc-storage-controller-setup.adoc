...
由 ONTAP Autonomous Ransomware Protection 创建的快照不能用于 SAP HANA 数据库的前向恢复。因此，不建议将其用于 SAP HANA 数据和日志卷，应为这些卷禁用此功能。请使用基于 SnapCenter 的数据保护和防篡改快照来进行勒索软件防护。link:../backup/hana-sc-generic-scope.html["使用 SnapCenter 进行 SAP HANA 备份和恢复"] ONTAP Autonomous Ransomware Protection 可能对其他卷（如 /hana/shared）有益。
