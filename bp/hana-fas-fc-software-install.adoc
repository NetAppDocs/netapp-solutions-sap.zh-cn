...
....
+
使用SAP `hdblcm`安装工具、通过在一个工作主机上运行以下命令来启动安装。使用 `addhosts`选项添加第二个工作空间(sapcc-hana TST-06)和备用主机(sapcc-hana TST-07)。CLI选项中包含了已准备文件的存储 `storage_cfg`目录 `global.ini`(`--storage_cfg=/hana/shared`)。根据所使用的操作系统版本，可能需要先安装 phyton 2.7 ，然后再安装 SAP HANA 数据库。
