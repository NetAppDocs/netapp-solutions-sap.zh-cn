...
* link:saphana_aff_fc_introduction.html["基于采用光纤通道协议的 NetApp AFF 系统的 SAP HANA"^]
* link:saphana-fas-nfs_introduction.html["基于采用 NFS 的 NetApp FAS 系统的 SAP HANA"^]
* link:saphana_aff_nfs_introduction.html["基于采用 NFS 的 NetApp AFF 系统的 SAP HANA"^]
...
可通过多种方法将存储连接到虚拟机（ VM ）。首选方法是，将具有 NFS 的存储卷直接从子操作系统中连接起来。link:saphana_aff_nfs_introduction.html["基于采用 NFS 的 NetApp AFF 系统的 SAP HANA"^]有关此选项的说明，请参见。
