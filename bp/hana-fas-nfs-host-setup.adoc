...
NOTE: 示例显示了使用 LSS 的 SID=NF2 的 SAP HANA 数据库。

要创建所需的子目录，请执行以下操作之一：

* 对于单主机系统，请挂载  `/hana/shared` 卷并创建  `shared`、 `lss-shared` 和  `usr-sap` 子目录。
...
....
* 对于多主机系统，请挂载 `/hana/shared`卷并为每个主机创建 `shared`、 `lss-shared`和 `usr-sap`子目录。
+
...
| /lss/shared | NFSv3 或 NFSv4 | NFSv3 或 NFSv4 
...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf`。此外、还必须调整服务器端值、并按照中所述设置会话插槽的数量link:hana-fas-nfs-storage-controller-setup.html#nfs-configuration-for-nfsv4["NFSv4 的 NFS 配置。"]
...
....
. 运行 `mount -a`以在所有主机上挂载文件系统。


下一个示例显示了一个 SID=NF2 的多主机 SAP HANA 数据库，其中 NFSv4.1 用于数据和日志文件系统，NFSv3 用于 `/hana/shared`、 `/lss/shared`和 `/usr/sap/NF2`文件系统。NFS 传输大小为读取使用 1MB，写入使用 256k。
