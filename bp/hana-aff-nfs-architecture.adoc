...
image:saphana_aff_nfs_image2a.png["八个连接到存储高可用性(HA)对的SAP HANA主机"]
...
image:saphana_aff_nfs_image3a.png["将其他SAP HANA主机连接到现有存储控制器HA对"]

无论部署的 AFF 系统如何， SAP HANA 环境也可以通过添加任何经过认证的存储控制器来进行扩展，以满足所需的节点密度，如下图所示。

image:saphana_aff_nfs_image4a.png["已添加其他HA对"]
...
image:saphana_aff_nfs_image6.png["图中显示了输入/输出对话框或表示已写入内容"]
