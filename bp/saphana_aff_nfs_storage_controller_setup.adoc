...
. 按照中所述检查正确的磁盘架配置 " shelf connection。 "
. 按照中所述创建和配置所需的聚合 " configuration。 "
. 创建 Storage Virtual Machine （ SVM ），如 " configuration。 "
. 按照中所述创建 LIF" configuration。 "
. 在聚合中创建卷，如 " configuration for SAP HANA single host systems，和 configuration for SAP HANA multiple host systems。 "
. 按照中所述设置所需的卷选项 " options。 "
. 按照中所述为 NFSv3 设置所需的选项 " configuration for NFSv3" 或 NFSv4 ，如 " configuration for NFSv4。 "
. 将卷挂载到命名空间并设置导出策略，如 " volumes to namespace and set export policies。 "
